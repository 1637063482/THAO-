...
> 审计日期：2026-07-18（结合项目所有者最新产品信息复核）
...
MyExpenseApp 2.0 是项目所有者为越南籍女朋友开发的私人单页 PWA。女朋友是唯一日常记账者，使用越南盾；项目所有者保留第二个既有 Firebase 账号用于查看和维护。系统不提供新用户注册、成员管理或公开服务。Firebase Authentication 控制两个既有账号登录，Firestore 的单个“年度共享账本文档”承担云端同步。

当前实现更准确的领域定义是：**年度共享收支矩阵**，而不是标准的“交易流水账”。每一天、每一支出分类只有一个聚合单元格；同日同分类的多次录入被拼成算术表达式。账户只体现为年初/年末四类余额快照，没有交易账户、转账、标签、账本成员、角色或审计日志。

项目优点是代码量小、上手成本低、交互闭环完整，已经具备登录、快速记账、实时同步、预算、统计图表、CNY 辅助展示、CSV 导出、JSON 导入、PWA 和连续记账激励。当前最直接的产品缺陷是连续记账状态不可靠且奖励无法到达；技术演进必须服务这一个私人应用，不能按通用财务 SaaS 扩张。
...
| 鉴权 | Firebase Authentication | `src/js/auth.js` | 仅两个既有账号邮箱密码登录/退出；应用内注册已移除 |
...
2. 新金额被追加为 `=<旧表达式>+<新金额>`。
3. 备注以逗号拼接到该日唯一备注字段。
4. 重新计算全年数据并触发保存；仅快速记账入口按“操作设备的今天”更新连续记账，直接表格编辑不会更新，这是已确认 Bug。

### 4.3 核心业务链路

- 登录：应用内注册入口已移除，仅两个既有 Firebase 账号可登录；实际线上读写权限最终由项目所有者已配置的 Firebase Rules 决定。
...
| 领域对象 | 当前状态 | 关键差距 |
|---|---|---|
| 用户 | 女朋友是唯一日常记账者；项目所有者查看/维护 | 两个既有账号由 Firebase 管理；应用不提供注册与成员管理 |
| 共享账本 | 唯一逻辑账本，按年度保存共享文档 | 不需要 Household、多租户或邀请；两账号是否读写同权必须以线上 Rules 为准 |
...
“分享”按钮只复制应用 URL，不承担账号授权职责；Firebase 权限仍是唯一授权来源。
...
- 产品范围已确认：账务事实以女朋友的 VND 记账为中心；项目所有者主要查看，CNY 只是显示换算；不支持其他家庭、公开注册或成员管理。
