...
function renderReminderDialog(reminders, locale, offline) {
  ...
  return `<div class="deposit-reminder-backdrop" data-reminder-backdrop><section class="deposit-reminder-dialog safe-area-bottom" role="dialog" aria-modal="true" aria-labelledby="deposit-reminder-title" tabindex="-1"><header><div><p class="deposit-eyebrow">${labels.D0}</p><h2 id="deposit-reminder-title">${labels.title}</h2><p>${labels.intro}</p></div><button type="button" class="deposit-form-close" data-close-reminders aria-label="${labels.close}"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><path d="M6 6l12 12M18 6L6 18"></path></svg></button></header>${offline ? `<p class="deposit-reminder-offline" role="status">${labels.offline}</p>` : ""}<ul>${items}</ul><footer><button type="button" class="btn-secondary" data-close-reminders>${labels.close}</button></footer></section></div>`;
}
