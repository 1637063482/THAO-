...
describe("ledger view toggle", () => {
  ...
  it("refreshes daily cards on the remote soft-update path and preserves pending input", async () => {
    ...
    state.activeYear = 2026;
    state.activeMonthId = 3;
    state.appState = { balances: {}, entries: { "3_1_dining": "100000", "3_1_transport": "80000", "3_5_remark": "later note" }, settings: {} };
    ...
    const focused = document.querySelector('[data-key="3_1_dining"]');
    focused.focus();
    focused.value = "999000";
    focused.dataset.raw = "999000";
    state.appState.entries["3_1_dining"] = "250000";
    state.appState.entries["3_1_transport"] = "180000";
    softUpdateDOM();
    expect(document.activeElement.dataset.key).toBe("3_1_dining");
    expect(document.querySelector('[data-key="3_1_dining"]').value).toBe("999000");
    expect(document.querySelector('[data-key="3_1_transport"]').value).toContain("180,000");
